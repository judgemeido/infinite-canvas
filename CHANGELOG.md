...
+ [调整] 应用更名为「以诗为墨·神笔绘卷」，全局背景改用半透明 bk.png。
+ [调整] 顶栏移除版本号与 GitHub 图标。
+ [调整] 生图工作台更名为「AIGC绘卷」，顶栏移除 Agent 与文档入口。
+ [调整] 工作台移除模型选择，模型统一在配置页设置。
+ [调整] AIGC绘卷工作台各面板与卡片改为透明背景，透出全局背景图。
+ [新增] 新增 Cloudflare Worker 反向代理（同源 /api 转发到 api.change2pro.com），部署后生图请求不再受浏览器跨域限制。
+ [新增] 新增互动闯关模式：独立关卡首页 + 关卡横幅，进入关卡自动预填起手提示词，生成成功即通关并弹庆祝；首关「以诗为墨·神笔绘卷」。
+ [调整] 闯关改为两关：第一关「诗词接龙」小学古诗选择小游戏，第二关「以诗为墨·神笔绘卷」生图挑战，两关均可自由体验。
+ [修复] 迁移旧配置：openai 渠道若持久化了直连 api.change2pro.com（或 api.openai.com）的地址，加载时自动改写为同源 /api 反代，修复老用户浏览器缓存旧地址导致跨域无法生图的问题。
+ [调整] 提示词库下线原有 7 个远程 GitHub 提示词来源，改为内置本地「诗词风景 · 灵感提示词」来源，收录 100 套诗词风景主题、25 种不同艺术风格的文字提示词，随站点静态资源分发。
+ [调整] 第一关「诗词接龙」改为按小学 1~6 年级分年级布置：进入后先选年级，题库按课本必背古诗扩充到 28 题，并为该关加上专属背景图。
+ [调整] 第一关更名为「古诗词大闯关」，覆盖 1~9 年级（含初中 7~9 年级），每局随机抽 10 题；基于 108 首课本古诗语料自动生成 7 种题型（诗词接龙、看意思猜古诗、识句猜篇名、看作者猜诗句、看诗句猜作者、看诗句猜朝代、多选古诗），各年级题库均达 100 题以上。
+ [新增] 古诗词大闯关答错时给出提示（诗句出处/篇名），并加入选项抖动、正确项弹跳、通关彩带等动效；页面背景改用 h.png 半透明（opacity 0.1）叠加暖色渐变，整体更精致。
+ [调整] 古诗词大闯关改为积分赛制：每局 15 题，每题仅一次作答机会，答错立即公布正确答案与出处，不再重试；答对每题得 10 分，可反复挑战不同年级并累计总分（本地持久化），选择页显示累计总分/已闯局数并支持一键清零。
+ [新增] 古诗词大闯关加入难度系统：选好年级后再选「简单 / 普通 / 困难」三档，难度越高题型越难（困难偏重看意思、看作者、猜朝代与多选），每题分值越高（×1.0 / ×1.5 / ×2.0）；每局结束由「AI 智能结算」特效（进度条 + 分步计算 + 分数滚动）按公平算法结算——本局公平积分 = 答对题数 × 每题分值 × 年级权重（1~9 年级 ×1.00~×1.40）× 准确率加成（满分 ×1.2、≥80% ×1.1）；页面「清零积分」升级为「重置关卡」，带确认弹窗一键清空累计积分与局数。
+ [修复] 关卡首页两张关卡卡片的「开始挑战/再玩一次」按钮因简介文字行数不同而未在底部对齐，改用 flex 撑满布局后按钮统一贴底、横向对齐。
+ [调整] 第二关「以诗为墨·神笔绘卷」生图工作台精简：移除参考图（图生图）功能，工作台改为纯文生图；宽高比去掉 2K/4K 高分辨率档位；生成张数只保留「1 张」快捷项与自定义输入，并在设置区加入「本站单次最多生成 20 张图」的文字提示（仅提示、不强制上限）。
